...
- **A terminal-like workspace without containers.** The agent gets a `bash` tool. `just-bash` provides shell behavior, while `@cloudflare/shell` provides the durable Workspace filesystem backed by Cloudflare storage.
...
- `lib/cloudflare-terminal.ts` - Connects Flue's `bash` tool to `just-bash` and `@cloudflare/shell`.
